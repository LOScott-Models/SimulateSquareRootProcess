...
__device__ inline double simulateQB(double x0, unsigned int seed[], double b, 
	double df_term, double sig_dt2, double dfterm2, double c, double mu_Beta, 
	double m2_Beta, double Beta_power)
{
	double x, sqrt_term, y, v, tem, tem2, quad_a, quad_b, quad_c, p_mix, b_mix, 
		lambda_nc, lambda_cf;
	...
	if (df_term >= 0.0) {
		...
		//	mean
			y = c * (b * x0 + dfterm2);
		//	variance
		//	v = c * c * 4.0 * sig_dt2 * sig_dt2 * (b * x0 + 0.5 * dfterm2);
			...
			if (p_mix >= 0.0 && p_mix <= 1.0) b_mix = (y - p_mix * mu_Beta) / 
													((1.0 - p_mix) * (lambda_nc + 1.0));
			else {
				p_mix = (-quad_b - sqrt(tem)) / (2.0 * quad_a);
			...
			if (tem <= p_mix) {
				...
				tem2 = sqrt(lambda_nc) + normcdfinv((1.0 - tem) / (1.0 - p_mix));
				x = b_mix * tem2 * tem2;
			}
			x = x / c;
...
__global__ void runQB(int nSim, int nT, int nTimeStepsPerDay, unsigned int* d_seeds, double *d_Sims,
	double x0, double b, double df_term, double sig_dt2, double dfterm2, double c,
	double mu_Beta, double m2_Beta, double Beta_power)
{
	int iSim;
	//Thread index
	...
	if (iSim < nSim) {
		...
		x = x0;
		for (i = 0; i < nT; i++) {
			for (j = 0; j < nTimeStepsPerDay; j++) {
				x = simulateQB(x, seed, b, df_term, sig_dt2, dfterm2, c, mu_Beta, m2_Beta,
					Beta_power);
			//	x = simulateQMBE(x, seed, b, df_term, sig_dt2, dfterm2, c, mu_Beta, m2_Beta,
			//		Beta_power);
			}
		}
		d_Sims[iSim] = x;
...
__global__ void runQE(int nSim, int nT, int nTimeStepsPerDay, unsigned int* d_seeds, double *d_Sims,
...
__global__ void runEuler(int nSim, int nT, int nTimeStepsPerDay, unsigned int* d_seeds, double* d_Sims,
...
__global__ void runNC_Chi(int nSim, int nT, int nTimeStepsPerDay, unsigned int* d_seeds, double* d_Sims,
