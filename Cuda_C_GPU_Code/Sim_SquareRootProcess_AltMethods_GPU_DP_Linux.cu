...
//
//	must add boost folder, in Eclipse right click on Project and go to Properties
//			-> C/C++ General -> Paths and Symbols -> Includes
//
//	This code runs double precision versions of the functions on GPU
//
//	Version 1
//	Revised November 2024
//	Up;oaded to GitHub, 7/19/2025
//
...
//	default values for tests runs
	...
	if (fin != NULL) {
		//err = fgets(HeaderLine, 200, fin);
		if (fgets(HeaderLine, 200, fin) != NULL) {
			err = fscanf(fin, "%lf,%lf,%lf,%lf,%lf", &kappa, &theta, &sigma, &lambda, &x0);
			for (i = 0; i < 6; i++) err = fscanf(fin, ",%u", &iseed[i]);
			fclose(fin);
		}
		else {
			printf(" Opened the parameter input file SquareRootProcessParams.csv, but file is empty \n");
		}
	}
	...
	double* Sims = new double[6 * (nSim)];
	double* CumDistFcn = new double[6 * (nSim)];
	double* h_Sims  = new double[(nSim)];
	float* Simsf = new float[4 * (nSim)];
	float* h_Simsf = new float[(nSim)];
...
//	Set up seedsd for GPU device
	unsigned int* h_seeds = new unsigned int[(6 * nSim)];
...
//	}
//	else {
		runQBNC1 <<<(1 + nSim / 128), 128 >>> (nSim, nSimsPerPath, d_seeds, d_Sims, x0, b, df_term,
...
//	}
	...
	if (df_term >= 0.0) {
		runFastQ << <(1 + nSim / 128), 128 >> > (nSim, nSimsPerPath, d_seeds, d_Sims, x0, b, df_term,
			sig_dt2, dfterm2, c, adj1, adj2);
	}
	else {
		runQBNC1 << <(1 + nSim / 128), 128 >> > (nSim, nSimsPerPath, d_seeds, d_Sims, x0, b, df_term,
	...
	runQB2Exp << <(1 + nSim / 128), 128 >> > (nSim, nSimsPerPath, d_seeds, d_Sims, x0, b,
	...
	runQE <<<(1 + nSim / 128), 128 >>> (nSim, nSimsPerPath, d_seeds, d_Sims, x0, b, QE_sig_dt2, dfterm2);
	...
	runEuler <<<(1 + nSim / 128), 128 >>> (nSim, nSimsPerPath, d_seeds, d_Sims, x0, b, kappa,
	...
	nSimsPerPath = nT * 8 * nTimeStepsPerDay;
	SkipAhead_MRG32k3a(nSimsPerPath, An1, An2);
	//	set seeds for the start of each path
	for (i = 0; i < 6; i++) {
		h_seeds[i] = iseed[i];
	...
	runNC_Chi << <(1 + nSim / 128), 128 >> > (nSim, (nT * nTimeStepsPerDay), d_seeds, d_Sims, x0, df, c,
