//============================================================================
// Name        : Sim_SquareRootProcesses_Linux.cpp
// Author      : LOS
// Version     :
// Copyright   : Your copyright notice
// Description : Main Program to Test Alternative Methods to Simulate
//						Square Root Processes
//============================================================================

// Sim_SquareRootProcesses_Linux.cpp : This file contains the 'main' function.
//	for Linux
//
...
int main()
{
	...
	char HeaderLine[500];
	double t_execute1, t_execute2, t_execute3, t_execute4, t_execute5;
	...
	fout2 = fopen("SimulatedData.txt", "w");
	if (fout2 == NULL) printf(" Cannot open output file SimulatedData.txt for writing output data \n");
	fout = fopen("TestNC_ChiSquared_Simulation.txt", "w");
	if (fout == NULL) printf(" Cannot open output file TestNC_ChiSquared_Simulation.txt for writing output \n");
	fout3 = fopen("SimulatedDistributionFunctions.txt", "w");
...
//	if (fin== NULL) printf(" Cannot open output file SquareRootProcessParams.csv \n");
	if (fin != NULL) {
		...
		for (i = 0; i < 6; i++) fscanf(fin, ",%u", &iseed[i]);
		fclose(fin);
	}
	...
	double* Sims = new double[8 * nSim];
	double* CumDistFcn = new double[8 * nSim];
    ...
    t_execute1 = difftime(t_end, t_start);
    ...
    t_execute2 = difftime(t_end, t_start);
    ...
    t_execute3 = difftime(t_end, t_start);
    ...
    t_execute4 = difftime(t_end, t_start);
	...
	printf("\n Runtimes: %s = %7.3f seconds, execution %7.3f,  %s = %7.3f seconds, execution %7.3f  \n",
		modelNames[0].data(), time1, t_execute1, modelNames[1].data(), time2, t_execute2);
	printf(" Runtimes: %s = %7.3f seconds, execution %7.3f,   %s = %7.3f seconds, execution %7.3f \n",
		modelNames[2].data(), time3, t_execute3, modelNames[3].data(), time4, t_execute4);

	fprintf(fout, "\n Runtimes: %s = %7.3f seconds, execution %7.3f,  %s = %7.3f seconds, execution %7.3f  \n",
		modelNames[0].data(), time1, t_execute1, modelNames[1].data(), time2, t_execute2);
	fprintf(fout, " Runtimes: %s = %7.3f seconds, execution %7.3f,   %s = %7.3f seconds, execution %7.3f \n",
		modelNames[2].data(), time3, t_execute3, modelNames[3].data(), time4, t_execute4);
	...
	//	Simulate the non central chi squared distribution
	    ...
	    t_execute5 = difftime(t_end, t_start);

		printf(" Runtimes: %s = %9.3f seconds, execution %9.2f \n", modelNames[5].data(), time5, t_execute5);
		fprintf(fout, " Runtimes: %s = %9.3f seconds, execution %9.2f \n", modelNames[5].data(), time5, t_execute5);
		...
		if (nTests > 6) {
		    ...
		    double t_execute6 = difftime(t_end, t_start);

			printf(" Runtimes: %s = %9.3f seconds, execution %9.2f \n", modelNames[6].data(), time6, t_execute6);
			fprintf(fout, " Runtimes: %s = %9.3f seconds, execution %9.2f \n", modelNames[6].data(), time6, t_execute6);
		}	//	end of if (nTests > 6)
	}	//	end of if (nTests > 5)
	...
	printf(" Run time for simulating exact non-central chi squared over longer time interval %8.3f seconds, execution time %8.2f \n\n", time5, t_execute5);
	fprintf(fout, " Run time for simulating exact non-central chi squared over longer time interval %8.3f  seconds, execution time %8.2f \n\n", time5, t_execute5);
