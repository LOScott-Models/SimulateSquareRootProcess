//============================================================================
// Name        : Test_NC_ChiSquareSimulators_Linux.cpp
// Author      : Louis Scott
// Version     :
// Copyright   : Your copyright notice
// Description : C++ Code to simulate square root processes
//============================================================================
//
//  This file contains the 'main' function. Program execution begins and ends there.
//
//	must add boost folder, highlight the project and go to File -> Properties ->
//	C/C++ General -> Paths and Symbols -> Includes -> GNU C++ -> Add the Boost folder
...
int main()
{
	int iT, i, j, k, nSim, nT, nTimeStepsPerDay, nDaysPerTimeUnit;
	...
	clock_t timestart, timeend1;
//	double time0, t_execute;
//    clock_t timeend2;
...
//	default values for tests runs
	...
	if (fin != NULL) {
		...
		fscanf(fin, "%lf,%lf,%lf,%lf,%lf", &kappa, &theta, &sigma, &lambda, &x0);
	...
	double* Sims = new double[5 * nSim];
	double* CumDistFcn = new double[5 * nSim];
...
//	set fixed parameters for non central chi squared simulation
	setchi2params(df, vb, vd, vp, nvexp, is_int, is_half_int);//	printf(" check: df %f vb %f vd %f vp %f nvexp %i is_int %i is_half_int %i \n", df, vb, vd, vp, nvexp, is_int, is_half_int);
	...
	printf(" Running quadratic approximation, with mixed beta-noncentral chi squared-1 (QB) \n");
	...
	for (i = 0; i < nSim; i++) {
		x = x0;
		for (iT = 0; iT < nT; iT++) {
			x = simulateQB(x, rdseed, b, df_term, sig_dt2, dfterm2, c, mu_Beta, m2_Beta,
				Beta_power);
		}
		Sims[i] = x;
	}
    ...
    timeend1 = clock();
    time1 = ((double) (timeend1 - timestart)) / CLOCKS_PER_SEC;
...
//	Andersen's quadratic exponential approximation
    ...
    timeend1 = clock();
    time4 = ((double) (timeend1 - timestart)) / CLOCKS_PER_SEC;
    ...
    timeend1 = clock();
    time2 = ((double) (timeend1 - timestart)) / CLOCKS_PER_SEC;
	...
	for (i = 0; i < nSim; i++) {
		...
		for (iT = 0; iT < nT; iT++) {
			...
			x = nc_chi2(x, df, c, b, tdseed, vb, vd, vp, nvexp, is_int, is_half_int,
						log_Factorial, wksp);
		}
		Sims[3*nSim + i] = x;
	}
    ...
    timeend1 = clock();
    time3 = ((double) (timeend1 - timestart)) / CLOCKS_PER_SEC;
...
//	statistics and other measures of goodness of fit
    ...
    timeend1 = clock();
    time5 = ((double) (timeend1 - timestart)) / CLOCKS_PER_SEC;
